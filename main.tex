...
% Slide 7: Using ULT Freezer Temperature-Energy Curve with enhanced TikZ visualization
\begin{frame}{Lab Sustainability in Action: ULT Freezers}
    \begin{columns}[T,onlytextwidth]
        \column{0.55\textwidth}
        % Custom TikZ visualization of freezer temperature vs energy consumption
        \begin{tikzpicture}[scale=0.75]
            % Define colors for visualization
            \definecolor{coldblue}{RGB}{0,102,204}
            \definecolor{energyred}{RGB}{204,0,0}
            
            % Setup coordinates
            \draw[thick, ->] (0,0) -- (8,0) node[right] {\textbf{Temperature (°C)}};
            \draw[thick, ->] (0,0) -- (0,6) node[above] {\textbf{Energy Use (kWh/day)}};
            
            % Temperature ticks and labels
            \foreach \x/\label in {1/-80, 3/-70, 5/-60, 7/-50}
                \draw[thick] (\x,0.1) -- (\x,-0.1) node[below] {\label};
            
            % Energy ticks and labels
            \foreach \y/\label in {1/5, 2.5/12, 4/19, 5.5/26}
                \draw[thick] (0.1,\y) -- (-0.1,\y) node[left] {\label};
            
            % Energy curve
            \draw[very thick, energyred] (1,5.5) .. controls (3,3.8) and (5,2.2) .. (7,1) 
                node[right, align=left] {\footnotesize \textbf{\textcolor{energyred}{Energy}}};
            
            % Fill area under curve
            \fill[energyred!20] (1,0) -- (1,5.5) .. controls (3,3.8) and (5,2.2) .. (7,1) -- (7,0) -- cycle;
            
            % Highlight optimal range
            \fill[coldblue!20] (3,0) rectangle (5,6);
            \node[align=center, text width=2cm] at (4,4.5) {\textbf{\textcolor{coldblue}{Optimal\\Range}}};
            
            % Add savings callout
            \node[draw=taskgreen, fill=taskgreen!10, rounded corners, text width=3cm, align=center, font=\small\bf] 
                at (6,4) {Moving from -80°C to -70°C saves 30\% energy!};
                
            % Add freezer icon
            \node at (1.5,2) {\scalebox{2}{\faSnowflake}};          
        \end{tikzpicture}
